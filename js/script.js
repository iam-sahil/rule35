...
if (apiSelect) {
  ...
          // Disable search for APIs other than Rule34, Danbooru, and Gelbooru
  ...
  async function fetchAutocompleteSuggestions(api, queryPrefix) {
    ...
        let autocompleteUrl = '';
        if (api === 'rule34') {
            autocompleteUrl = `https://api.rule34.xxx/autocomplete.php?q=${encodeURIComponent(queryPrefix)}`;
        } else {
...
    if (selectedApi !== 'rule34') { // Autocomplete only for Rule34
        autocompleteList.style.display = 'none';
        return;
    }

    const queryPrefix = this.value.trim();
    if (queryPrefix.length < 1) { // Start suggesting after 2 characters
        autocompleteList.style.display = 'none';
        return;
    }

    const suggestions = await fetchAutocompleteSuggestions(selectedApi, queryPrefix);
...
    const proxiedUrl = useProxy ? `https://api.allorigins.win/get?url=${encodeURIComponent(url)}` : url;
...
        if (selectedApi === 'konachan' && typeof data === 'string') { // Corrected condition
            try {
...
            }
        }  else if (selectedApi === 'gelbooru') {
            if (data && data.post) { // Gelbooru wraps images in a 'post' array
                images = data.post.map(function(img) { return normalizeImage(img); });
            } else if (Array.isArray(data)) { // Fallback in case API changes structure
...
            images = data.map(function(img) { return normalizeImage(img); });
        } else if (data && Array.isArray(data.post)) { // For APIs like Gelbooru that might wrap in 'post'
            images = data.post.map(function(img) { return normalizeImage(img); });
        }
        else if (selectedApi !== 'waifu' && (!Array.isArray(data) || data.length === 0)) { // Check for empty array for non-waifu APIs
...
        // Extract filename from URL or default to image ID
...
            filename += '.jpeg';
        } else if (filename.split('.').pop().length > 4) { // Very long extension is suspicious
...
function handleImageClick(e, imageData, container) {
  ...
  if (selectedApi === "rule34" || selectedApi === "danbooru" || selectedApi === "konachan" || selectedApi === "gelbooru" || selectedApi === "yande") {
