export default async function handler(req, res) {
    ...
    if (imageUrl) {
        try {
            ...
            if (!imageResponse.ok) {
                throw new Error(`Failed to fetch image. Status: ${imageResponse.status} ${imageResponse.statusText}`);
            }
            ...
            return res.status(500).json({ error: 'Failed to fetch image from the source for proxy.', details: error.message });
        }
        return;
    }
    ...
    if (api === 'rule34') {
        ...
        url = `https://gelbooru.com/index.php?page=dapi&s=post&q=index&json=1&limit=50&pid=${page}&tags=${encodeURIComponent(query)}`; // pid for page
    ...
    try {
        ...
        if (api !== 'gelbooru' && api !== 'konachan' && !Array.isArray(data)) { // Adjust array check for APIs expecting arrays
            return res.status(500).json({ error: `API response is not an array for ${api}` });
        }
        if (api === 'gelbooru') {
            if (!data.post && !Array.isArray(data)) { // Check for 'post' array or fallback to array if API changes
                return res.status(500).json({ error: `Gelbooru API response is not in expected format` });
            }
        }
        if (api === 'konachan' && typeof data !== 'string' && !Array.isArray(data)) { // Konachan might return string or array
             return res.status(500).json({ error: `Konachan API response is not in expected format (string or array)` });
        }
        ...
        if (error.message.startsWith('Failed to fetch data from the API')) {
            return res.status(502).json({ error: `Bad Gateway: External API (${api}) is down or not responding. Details: ${error.message}` });
        }
        res.status(500).json({ error: `Error fetching data from the API (${api}).`, details: error.message });
    }
}